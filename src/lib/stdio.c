...
static char pintos_buffer[1000];
...
{
#ifdef RRDEBUG || RRINFO
  va_list args;
  int len = 0;
  va_start (args, format);
  len += vsnprintf(&pintos_buffer[len], 1000, format, args);
  va_end (args);

  int i;
  for(i=0;i<len;i++) outb(PINTOS_PORT, pintos_buffer[i]);
  outb(PINTOS_PORT, '\0');
...
int myprintf_error(const char *format, ...)
{
#ifdef RRERROR || RRDEBUG || RRINFO
  va_list args;
  va_start (args, format);
  int result = myprintf(format, args);
  va_end (args);
  return result;
#else
  return 0;
...
int myprintf_info(const char *format, ...)
{
#ifdef RRINFO
  va_list args;
  va_start (args, format);
...
/* Writes formatted output to the console.
   ...
   In userspace, the console is file descriptor 1. */
int
mydumbprintf (const char *buffer) 
{
  int len = strlen(buffer);

  int i;
  for(i=0;i<len;i++) outb(PINTOS_PORT, buffer[i]);
  outb(PINTOS_PORT, '\0');
